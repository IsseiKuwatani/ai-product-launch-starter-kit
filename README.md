...
2. [/setup対応 skill の説明ページ](docs/manuals/setup-skill.html)
3. [開発プロンプト集 完全版 HTML](prompts/development-prompts.html)
   - Supabase/Vercel前提なし
   - ローカルで動くMVP制作
   - localStorage / JSON / CSV保存
   - AI API追加前の設計
   - LP・フォーム・結果画面改善
4. [skills・エージェント設計集](skills/03-skills-and-agents.md)
5. [AIプロダクト企画・要件定義ワークシート](docs/04-product-planning-worksheet.md)
6. [制作テンプレート: Next.js AI Product Starter](template/next-ai-product-starter/)

## おすすめの使い方

1. まず `docs/04-product-planning-worksheet.md` で作りたいものを1ページに整理する
2. Claude Code / Codex の導入マニュアルに沿って開発環境を整える
3. `template/next-ai-product-starter/` を自分のPCまたはリポジトリで開く
4. Claude Codeで `/setup` を実行する
5. 3つの質問に答えて、1画面MVPへ編集する
6. 必要に応じて `prompts/02-development-prompts.md` のプロンプトや `skills/03-skills-and-agents.md` を使う
...
Claude Codeでテンプレートを開き、`/setup` を実行すると、初回起動・ヒアリング・ローカルで動くMVP化の手順を進められます。Supabase/Vercel/外部DB/ログイン/決済は前提にしません。
